...
interface Props {
    onClose: () => void
    onImport: (entries: Omit<RadaiEntry, 'id'>[]) => void
}
...
// Proper Excel TSV parser — handles quoted cells with embedded newlines and tabs.
// Excel wraps multi-line cells in double-quotes; "" inside a quoted cell is a literal quote.
...
// Parse each line of an amount cell to a clean number string; join with \n to preserve multi-line.
...
// Sheet column order (0-indexed):
// 0: Date  1: Serial No.  2: DV/Payroll No.  3: ORS/BURS No.
// 4: RC Code  5: Payee  6: UACS Object Code  7: Nature of Payment  8: Amount
//
// Continuation rows carry extra UACS/Amount lines for the previous entry.
// Identified by: no Serial No. (col 1) — every real entry has one.
// Col 0 may be empty, repeated, or contain stray text on continuation rows.
function parseTsv(raw: string): ParsedRow[] {
    ...
    for (let i = 0; i < grid.length; i++) {
        ...
        // ── Continuation row ────────────────────────────────────────────────────
        // No serial number = not a main entry. If it also has UACS or amount,
        // fold those values into the previous row.
        ...
        if (cols.length < 2) {
            result.push({
                date: '', serial_no: '', dv_payroll_no: '', ors_burs_no: '',
                responsibility_center_code: '', payee: '', uacs_object_code: '',
                nature_of_payment: '', amount: '',
                error: `Row ${i + 1}: not enough columns (got ${cols.length})`,
            })
            continue
        }

        const isoDate = parseDate(rawDate)
        const rawAmount = cols[8]?.trim() ?? ''
...
export default function PasteImportDialog({ onClose, onImport }: Props) {
    const [mds_type, setMdsType] = useState<'regular' | 'special'>('regular')
            ...
            <div className="bg-card border border-border rounded-xl w-full max-w-4xl shadow-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="px-6 py-5 flex flex-col gap-5 overflow-y-auto">
                    ...
                    <div className="flex gap-2">
                        {([['regular', 'RADAI-MDS REGULAR'], ['special', 'RADAI-MT SPECIAL']] as const).map(([val, label]) => (
                            <button
                                key={val}
                                type="button"
                                onClick={() => setMdsType(val)}
                    ...
                    <div>
                        <p className="text-xs text-muted-foreground mb-2">
                            Select and copy rows from the RADAI table in Excel, then paste below.
                            Columns must be in order:{' '}
                            <span className="font-gmedium text-foreground">
                                Date → Serial No. → DV/Payroll No. → ORS/BURS No. → RC Code → Payee → UACS Code → Nature of Payment → Amount
                            </span>
                        </p>
                        <textarea
                        ...
                        <div>
                            <div className="flex items-center justify-between border-b border-border pb-1 mb-3">
                                ...
                                <div className="flex items-center gap-3 text-xs">
                                    {validRows.length > 0 && (
                                        <span className="flex items-center gap-1 text-green-600 dark:text-green-400 font-gmedium">
                                            <CheckCircle2 size={12} /> {validRows.length} valid
                                        </span>
                                    )}
                                    {errorRows.length > 0 && (
                                        <span className="flex items-center gap-1 text-destructive font-gmedium">
                                            <AlertCircle size={12} /> {errorRows.length} skipped
                                        </span>
                                    )}
                                </div>
                            </div>
                            {errorRows.map((r, i) => (
                                <p key={i} className="text-xs text-destructive mb-1">{r.error}</p>
                            ))}
                            {validRows.length > 0 && (
                                        ...
                                        <tbody>
                                            {validRows.map((r, i) => (
                                                <tr key={i} className="border-t border-border hover:bg-muted/30 transition-colors">
                                                    ...
                                                    <td className="px-3 py-2 text-right font-gmedium">
                                                        <AmountLines value={r.amount} />
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
...
function AmountLines({ value }: { value: string }) {
    ...
    if (lines.length <= 1) return <span className="whitespace-nowrap">{value ? fmt(lines[0]) : '—'}</span>
    return (
        <div className="flex flex-col gap-0.5 items-end">
            {lines.map((l, i) => <span key={i} className="whitespace-nowrap">{fmt(l)}</span>)}
        </div>
    )
}
