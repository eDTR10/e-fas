...
import EditRciDialog from './EditRciDialog'
...
const STORAGE_KEY = 'efas_rci_entries'

function loadEntries(): RciEntry[] {
    try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]') } catch { return [] }
}
function saveEntries(entries: RciEntry[]) { localStorage.setItem(STORAGE_KEY, JSON.stringify(entries)) }
...
const TYPE_META: Record<'regular' | 'special' | 'tf', { label: string; badge: string; accent: string }> = {
    regular: { label: 'RCI-MDS REGULAR', badge: 'MDS Reg', accent: 'text-blue-600 dark:text-blue-400' },
    special: { label: 'RCI-MT SPECIAL',  badge: 'MT Spl',  accent: 'text-purple-600 dark:text-purple-400' },
    tf:      { label: 'RCI-TF CHECKS',   badge: 'TF',      accent: 'text-teal-600 dark:text-teal-400' },
}

const BADGE_CLS: Record<'regular' | 'special' | 'tf', string> = {
    ...
    tf:      'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400',
}

// ─── Main ─────────────────────────────────────────────────────────────────────

export default function RCIContainer() {
    const [entries, setEntries] = useState<RciEntry[]>(loadEntries)
    const [search, setSearch] = useState('')
    const [filterMonth, setFilterMonth] = useState('')
    const [filterType, setFilterType] = useState<'' | 'regular' | 'special' | 'tf'>('')
    const [showArchived, setShowArchived] = useState(false)
    const [selected, setSelected] = useState<Set<number>>(new Set())
    const [showAdd, setShowAdd] = useState(false)
    const [showPaste, setShowPaste] = useState(false)
    const [showGenerate, setShowGenerate] = useState(false)
    const [editTarget, setEditTarget] = useState<RciEntry | null>(null)
    const [viewEntry, setViewEntry] = useState<RciEntry | null>(null)
    ...
    const onDragStart = (e: React.MouseEvent) => {
        ...
        isDragging.current = true; dragStartX.current = e.clientX; scrollStartX.current = tableRef.current?.scrollLeft ?? 0
    }
    ...
    const persist = (updated: RciEntry[]) => { setEntries(updated); saveEntries(updated) }

    const handleAdd = (e: RciEntry) => {
        persist([...entries, { ...e, period_covered: derivePeriod(e.date), id: nextId() }])
        setShowAdd(false)
    }

    const handlePasteImport = (newEntries: Omit<RciEntry, 'id'>[]) => {
        const withIds = newEntries.map(e => ({ ...e, period_covered: derivePeriod(e.date), id: nextId() }))
        persist([...entries, ...withIds])
        setShowPaste(false)
    }

    const handleEdit = (updated: RciEntry) => {
    ...
    const handleDelete = (id: number) => {
        if (!window.confirm('Delete this RCI entry?')) return
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-3 gap-3">
                <TypeCard label="RCI-MDS REGULAR" count={regularEntries.length} amount={regularTotal} accent={TYPE_META.regular.accent} />
                <TypeCard label="RCI-MT SPECIAL"  count={specialEntries.length} amount={specialTotal} accent={TYPE_META.special.accent} />
                <TypeCard label="RCI-TF CHECKS"   count={tfEntries.length}      amount={tfTotal}      accent={TYPE_META.tf.accent} />
            </div>
            <div className="grid grid-cols-3 gap-3">
            ...
            <div className="flex flex-wrap gap-2 items-center justify-between">
                <div className="flex flex-wrap gap-2 items-center">
                    ...
                    {([['', 'All'], ['regular', 'MDS Regular'], ['special', 'MT Special'], ['tf', 'TF Checks']] as const).map(([val, label]) => (
                ...
                <div className="flex gap-2">
                    <button onClick={() => { if (filtered.length === 0) { alert('No entries to generate a report for.'); return }; setShowGenerate(true) }}
            ...
            <div className="flex gap-4 items-start">
                <div className="flex-1 min-w-0">
                    <div ref={tableRef} className="overflow-x-auto rounded-xl border border-border bg-card cursor-grab active:cursor-grabbing select-none"
                                ...
                                {filtered.length === 0 ? (
                                    <tr>
                                        <td colSpan={13} className="text-center py-10 text-muted-foreground text-sm">
                                            {showArchived ? 'No archived entries.' : hasFilter ? 'No entries match the current filters.' : 'No RCI entries yet. Click "Add Entry" to begin.'}
                                        </td>
                                    </tr>
                                ) : (
                                    filtered.map(e => {
                                        const type = (e.mds_type ?? 'regular') as 'regular' | 'special' | 'tf'
                                        const isViewed = viewEntry?.id === e.id
                                        return (
                                            <tr key={e.id} onClick={() => setViewEntry(isViewed ? null : e)}
                                                ...
                                                <td className="px-4 py-3 w-10" onClick={ev => ev.stopPropagation()}>
                                                    <input type="checkbox" checked={effectiveSelected.has(e.id)} onChange={() => toggleOne(e.id)} className="accent-primary cursor-pointer" />
                                                </td>
                                                <Td>
                                                    <span className={`text-xs font-gmedium px-2 py-0.5 rounded-full ${BADGE_CLS[type]}`}>
                                                ...
                                                <Td className="max-w-[160px]"><span className="line-clamp-2 text-xs" title={e.payee}>{e.payee}</span></Td>
                                                <Td className="font-mono text-xs"><MultiLine value={e.uacs_object_code} /></Td>
                                                <Td className="max-w-[220px]"><span className="line-clamp-2 text-xs" title={e.nature_of_payment}>{e.nature_of_payment}</span></Td>
                                                ...
                                                <Td>
                                                    <div className="flex items-center gap-1.5" onClick={ev => ev.stopPropagation()}>
                                                        {!e.archived && <ActionBtn title="Edit" onClick={() => setEditTarget(e)}><Pencil size={13} /></ActionBtn>}
                                                        <ActionBtn title="Delete" danger onClick={() => handleDelete(e.id)}><Trash2 size={13} /></ActionBtn>
                                                    </div>
                                                </Td>
                                            </tr>
                                        )
                ...
                {viewEntry && (
                    <div className="flex shrink-0" style={{ width: panelWidth }}>
                        <div className="w-1 self-stretch cursor-col-resize hover:bg-primary/40 transition rounded-full mr-1"
                            onMouseDown={e => { e.preventDefault(); isResizing.current = true; resizeStartX.current = e.clientX; resizeStartW.current = panelWidth }} />
                        <div className="flex-1 rounded-xl border border-border bg-card overflow-hidden flex flex-col" style={{ maxHeight: 'calc(100vh - 180px)' }}>
                            <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-card shrink-0">
                                <span className={`text-xs font-gsemibold px-2.5 py-1 rounded-full ${BADGE_CLS[(viewEntry.mds_type ?? 'regular') as 'regular' | 'special' | 'tf']}`}>
                                    {TYPE_META[(viewEntry.mds_type ?? 'regular') as 'regular' | 'special' | 'tf'].label}
                                </span>
                                <div className="flex items-center gap-0.5">
                                    <button disabled={!canPrev} onClick={() => setViewEntry(filtered[viewIdx - 1])} title="Previous"
                                    ...
                                    <button disabled={!canNext} onClick={() => setViewEntry(filtered[viewIdx + 1])} title="Next"
                                        className="p-1.5 rounded hover:bg-muted transition text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed">
                            ...
                            <div className="overflow-y-auto flex-1 px-4 py-4 flex flex-col gap-5">
                                ...
                                <PanelSection title="Report Header">
                                    ...
                                    {(viewEntry.check_nos_from || viewEntry.check_nos_to) && (
                                        <PanelRow label="Check Nos."
                                            value={[viewEntry.check_nos_from, viewEntry.check_nos_to].filter(Boolean).join(' – ')} />
                                    )}
                                </PanelSection>
            ...
            {showAdd && <AddRciDialog onClose={() => setShowAdd(false)} onSave={handleAdd} />}
            ...
            {editTarget && <EditRciDialog entry={editTarget} onClose={() => setEditTarget(null)} onSave={handleEdit} />}
        </>
    )
}
...
function AmountLines({ value }: { value: string }) {
    ...
    return <div className="flex flex-col gap-0.5 items-end">{lines.map((l, i) => <span key={i} className="whitespace-nowrap">{fmtPHP(parseFloat(l))}</span>)}</div>
}

function MultiLine({ value }: { value: string }) {
    const lines = value ? value.split('\n').filter(Boolean) : []
    if (lines.length <= 1) return <span className="whitespace-nowrap">{value || '—'}</span>
    return <div className="flex flex-col gap-0.5">{lines.map((l, i) => <span key={i} className="whitespace-nowrap">{l}</span>)}</div>
}
...
function PanelRow({ label, value, wrap, muted, children }: { label: string; value?: string; wrap?: boolean; muted?: boolean; children?: React.ReactNode }) {
    return (
        <div className="flex gap-2 text-xs">
            {label && <span className="text-muted-foreground shrink-0 w-28 pt-0.5">{label}</span>}
            {children ?? (
                <span className={`font-gmedium break-words min-w-0 ${muted ? 'text-muted-foreground italic' : 'text-foreground'} ${wrap ? '' : 'whitespace-nowrap overflow-hidden text-ellipsis'}`}>
                    {value || '—'}
                </span>
            )}
        </div>
...
function MultiLinePanelValue({ value }: { value: string }) {
    ...
    return <div className="flex flex-col gap-0.5 min-w-0">{lines.map((l, i) => <span key={i} className="text-xs font-mono text-foreground">{l}</span>)}</div>
}
                ...
                {amtLines.length > 1 && (
                    <tfoot>
                        <tr className="border-t-2 border-border bg-muted/30">
                            ...
                            <td className="px-3 py-1.5 text-right font-gbold text-foreground whitespace-nowrap">₱{fmtPHP(total)}</td>
                        </tr>
                    </tfoot>
                )}
            </table>
